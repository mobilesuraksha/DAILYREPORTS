...
window.appLogin = async () => {
    ...
    catch (error) { document.getElementById('loginMsg').textContent = "Error: " + error.message; }
};
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.payment-select').forEach(sel => {
        sel.innerHTML = ["Cash", "UPI", "Counter Pay", "Bank Transfer", "Card", "Cheque", "Other"].map(m => `<option value="${m}">${m}</option>`).join('');
    });
...
window.addBoxGroup = (data={}) => {
    ...
    div.innerHTML = `<div class="box-group-header"><div class="box-group-title">Box Config</div><button type="button" class="btn btn-sm btn-danger del-grp">Remove</button></div>
    ...
    if(data.items) data.items.forEach(i => window.addItemToGroup(null, div.querySelector('.items-wrapper'), i));
    else window.addItemToGroup(div.querySelector('.add-itm'));
}
...
document.getElementById('orderForm').addEventListener('submit', async(e) => {
    e.preventDefault();
    const no = document.getElementById('orderNo').value.trim(), cust = document.getElementById('customerName').value.trim(), mob = document.getElementById('mobile').value.trim();
    const exist = globalOrders.find(o => o.no === no && o.cust.toLowerCase() === cust.toLowerCase());
    let merge = false;
    if(exist && !document.getElementById('orderId').value) { if(confirm(`Order #${no} exists. Merge items?`)) merge = true; else merge = false; }
    ...
    try {
        if(merge) {
            ...
            const lid = genId();
            await setDoc(doc(db, "ledger", lid), { id: lid, type: 'DEBIT', orderId: exist.id, cust: exist.cust, date: document.getElementById('orderDate').value, amount: tot, desc: `Order #${no} (Merged)`, itemSnapshot: bgs });
            alert("Merged!");
        } else {
            const isEdit = document.getElementById('orderId').value !== "";
            const ord = { id: id, no: no, date: document.getElementById('orderDate').value, delDate: document.getElementById('deliveryDate').value, delTime: document.getElementById('deliveryTime').value, cust: cust, mobile: mob, type: document.getElementById('orderType').value, note: document.getElementById('orderRemark').value, boxGroups: bgs, subTotal: parseFloat(document.getElementById('subTotal').value), discount: parseFloat(document.getElementById('discount').value), total: tot };
            if(isEdit) {
                await updateDoc(doc(db, "orders", id), ord);
                const l = globalLedger.find(x => x.orderId === id && x.type === 'DEBIT');
                if(l) await updateDoc(doc(db, "ledger", l.id), { amount: tot, cust: cust, date: ord.date, itemSnapshot: bgs });
                alert("Updated!");
            } else {
                await setDoc(doc(db, "orders", id), ord);
                const lid = genId();
                await setDoc(doc(db, "ledger", lid), { id: lid, type: 'DEBIT', orderId: id, cust: cust, date: ord.date, amount: tot, desc: `Order #${no}`, itemSnapshot: bgs });
                alert("Saved!");
            }
        }
        ...
        document.getElementById('orderForm').reset(); document.getElementById('boxContainer').innerHTML=''; window.addBoxGroup();
    } catch(err) { alert(err.message); }
});

// --- PRODUCTION (ADV 2 DAYS) ---
window.renderProduction = () => {
    const d = document.getElementById('prodDateFilter').value;
    const target = window.addDays(d, 2);
    ...
    globalOrders.forEach(o => {
        o.boxGroups.forEach(bg => {
            ...
            const ed = bg.groupDelDate || o.delDate;
            if(ed === target) {
                const fac = parseFloat(bg.groupQty)||0;
                bg.items.forEach(i => {
                    if(q && !i.name.toLowerCase().includes(q)) return;
                    if(!summary[i.name]) summary[i.name] = { qty:0, rem: new Set(), date: ed };
    ...
    if(Object.keys(summary).length===0) tb.innerHTML='<tr><td colspan="4">No orders due on target date.</td></tr>';
    for(const [n, v] of Object.entries(summary)) tb.innerHTML+=`<tr><td>${n}</td><td style="font-size:16px; font-weight:bold; color:#d35400">${v.qty.toFixed(2)}</td><td>${window.formatDate(v.date)}</td><td>${Array.from(v.rem).join(', ')}</td></tr>`;
}

// --- PACKING (ADV 1 DAY) ---
window.renderPacking = () => {
    const d = document.getElementById('packDateFilter').value;
    const target = window.addDays(d, 1);
    ...
    globalOrders.sort((a,b)=>(a.delTime||'').localeCompare(b.delTime||'')).forEach(o => {
        ...
        o.boxGroups.forEach(bg => {
            ...
            if(ed === target) {
                has = true;
                const tm = bg.groupDelTime ? ` (@ ${window.formatAMPM(bg.groupDelTime)})` : '';
...
window.renderMasterSummary = () => {
    ...
    globalOrders.forEach(o => {
        o.boxGroups.forEach((bg, idx) => {
            ev.push({ oid:o.id, idx:idx, d: bg.groupDelDate||o.delDate, t: bg.groupDelTime||o.delTime, no:o.no, c:o.cust, m:o.mobile, g:bg.groupName, q:bg.groupQty, i:bg.items, s:bg.status||'Pending' });
        });
    });
...
window.renderSplitManager = () => {
    ...
    matches.forEach(o => {
        let tq=0, pq=0, dq=0;
        o.boxGroups.forEach(bg=>{ const q=parseFloat(bg.groupQty)||0; tq+=q; if(bg.status==='Delivered' || (bg.status!=='Cancelled' && (bg.groupDelDate||o.delDate)<today)) dq+=q; else if(bg.status!=='Cancelled') pq+=q; });
        ...
        o.boxGroups.forEach((bg,idx)=>{
            ...
            h+=`<tr><td style="border-bottom:1px solid #eee"><b>${bg.groupName}</b> (${bg.status})<br>Qty: ${bg.groupQty}</td>
            <td style="border-bottom:1px solid #eee"><input type="date" id="d-u-${o.id}-${idx}" value="${ed}" style="width:90px" ${dis}><input type="time" id="t-u-${o.id}-${idx}" value="${et}" style="width:70px" ${dis}><button class="btn btn-sm btn-primary" onclick="window.updateSplitDate('${o.id}',${idx},'d-u-${o.id}-${idx}','t-u-${o.id}-${idx}')" ${dis}>Upd</button></td>
            <td style="border-bottom:1px solid #eee"><input type="number" id="q-s-${o.id}-${idx}" style="width:50px" ${dis}></td>
            <td style="border-bottom:1px solid #eee"><input type="date" id="d-s-${o.id}-${idx}" style="width:90px" ${dis}><input type="time" id="t-s-${o.id}-${idx}" style="width:70px" ${dis}></td>
            <td style="border-bottom:1px solid #eee"><button class="btn btn-sm btn-warning" onclick="window.splitOrderGroup('${o.id}',${idx},'q-s-${o.id}-${idx}','d-s-${o.id}-${idx}','t-s-${o.id}-${idx}')" ${dis}>Split</button></td></tr>`;
        });
        h+='</tbody></table>';
        const card = document.createElement('div'); card.className='admin-card'; card.style.textAlign='left';
        card.innerHTML = `<div style="display:flex;justify-content:space-between"><h3>${o.cust} <small>#${o.no}</small></h3><div><span class="stat-badge">Total:${tq}</span> <span class="stat-badge" style="color:green">Done:${dq}</span></div></div>${h}`;
        c.appendChild(card);
    });
}
...
window.renderSummary = () => { /* Standard Summary Logic */ 
...
window.savePayment = async () => { /* Standard Payment Logic */ 
    const id = document.getElementById('modalOrderId').value; const amt = parseFloat(document.getElementById('modalPayAmt').value); const disc = parseFloat(document.getElementById('modalDiscount').value); const mode = document.getElementById('modalPayMode').value; const d = document.getElementById('modalPayDate').value; const o = globalOrders.find(x => x.id === id);
